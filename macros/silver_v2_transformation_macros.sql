...
{% macro transform_string2(selector, column_name) %}
    {% if selector == 1 %}
        TRIM({{ column_name }})
...
{% macro transform_numeric2(selector, column_name) %}
    {% if selector == 1 %}
        CASE 
...
{% macro transform_datetime2(selector, column_name, is_join_key=false) %}
    {% if selector == 1 %}
        CASE 
