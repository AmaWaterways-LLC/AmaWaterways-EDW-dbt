    ...
    config(
        ...
        pre_hook=[
            "{% set target_relation = adapter.get_relation(database=this.database, schema=this.schema, identifier=this.name) %}
             ...
             {% if table_exists %}
                 {% set cfg = get_config_row('SW1', this.database, this.schema, 'SAIL_HEADER') %}
                 {% set load_type_val = 'FULL' if cfg['LAST_UPDATED_WATERMARK_VALUE'] is none else 'INCREMENTAL' %}            
             {% endif %}"
        ],
        post_hook=[
            "{% if execute %}
                 {% set wm_col_sw1 = get_watermark_column('SW1', this.database, this.schema, 'SAIL_HEADER') %}
                 {% set max_wm_sw1 = compute_max_watermark_seaware(this, wm_col_sw1, 'SW1') %}
                 {% if max_wm_sw1 is not none %}
                     {% do update_config_watermark('SW1', this.database, this.schema, 'SAIL_HEADER', max_wm_sw1) %}
                 {% endif %}

                 {% set wm_col_sw2 = get_watermark_column('SW2', this.database, this.schema, 'SAIL_HEADER') %}
                 {% set max_wm_sw2 = compute_max_watermark_seaware(this, wm_col_sw2, 'SW2') %}
                 {% if max_wm_sw2 is not none %}
                     {% do update_config_watermark('SW2', this.database, this.schema, 'SAIL_HEADER', max_wm_sw2) %}
                {% endif %}
             {% endif %}"
        ]
    )
...
{% if execute %}
    {% set cfg_sw1 = get_config_row('SW1', this.database, this.schema, 'SAIL_HEADER') %}
    {% set wm_col_sw1 = cfg_sw1['WATERMARK_COLUMN'] %}
    {% set last_wm_sw1 = cfg_sw1['LAST_UPDATED_WATERMARK_VALUE'] %}
    {% set is_full_sw1 = (last_wm_sw1 is none) %}
    {% set cfg_sw2 = get_config_row('SW2', this.database, this.schema, 'SAIL_HEADER') %}
...
WITH sw1_src AS (
    ...
    FROM {{ source(var('bronze_source_prefix') ~ '_SW1', 'SAIL_HEADER') }}
...
sw2_src AS (
    ...
    FROM {{ source(var('bronze_source_prefix') ~ '_SW2', 'SAIL_HEADER') }}
    -- Incremental load: include only rows whose watermark is greater than the last recorded watermark value
    {% if is_incremental() and not is_full %}
...
QUALIFY
    ROW_NUMBER() OVER (
        PARTITION BY SAIL_ID, DATA_SOURCE
