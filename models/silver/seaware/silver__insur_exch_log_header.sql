...
{{
    config(
        ...
        unique_key=['_FIVETRAN_ID', 'LOG_HEADER_ID'],
        pre_hook=[
            "{% set target_relation = adapter.get_relation(database=this.database, schema=this.schema, identifier=this.name) %}
             ...
             {% if table_exists %}
                 ...
                 {% do audit_start(
                     pipeline_name='bronze_to_silver_insur_exch_log_header',
                     source_name='AUTO',
                     database_name=target.database,
                     schema_name=target.schema,
                     table_name='INSUR_EXCH_LOG_HEADER',
                     layer='SILVER',
        ...
        post_hook=[
            ...
            "{% if execute %}
                 ...
                 {% if max_wm_sw1 is not none %}
                     {% do update_config_watermark('SW1', target.database, target.schema, 'INSUR_EXCH_LOG_HEADER', max_wm) %}
                 {% endif %}

                {% set wm_col_sw2 = get_watermark_column('SW2', target.database, target.schema, 'INSUR_EXCH_LOG_HEADER') %}
                 {% set max_wm_sw2 = compute_max_watermark_seaware(this, wm_col_sw2, 'SW2') %}
                 {% if max_wm_sw2 is not none %}
                     {% do update_config_watermark('SW2', target.database, target.schema, 'INSUR_EXCH_LOG_HEADER', max_wm) %}
                 {% endif %}
             {% endif %}"
        ]
    )
