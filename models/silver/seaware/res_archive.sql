...
WITH sw1_src AS (
    SELECT
            ...
            {{ transform_date('RES_INIT_DATE') }} AS RES_INIT_DATE,
            ...
            {{ transform_date('VACATION_DATE') }} AS VACATION_DATE,
            {{ transform_date('VACATION_END') }} AS VACATION_END,
    ...
    FROM {{ source(var('bronze_source_prefix') ~ '_SW1', 'RES_ARCHIVE') }}
...
sw2_src AS (
    SELECT
            ...
            {{ transform_datetime('_FIVETRAN_SYNCED') }} AS LAST_UPDATED_TIMESTAMP,
            _FIVETRAN_DELETED AS SOURCE_DELETED
    FROM {{ source(var('bronze_source_prefix') ~ '_SW2', 'RES_ARCHIVE') }}
    -- Incremental load: include only rows whose watermark is greater than the last recorded watermark value
    {% if is_incremental() and not is_full %}
...
FROM (
    SELECT
        {{ dbt_utils.generate_surrogate_key(["RECORD_ID", "DATA_SOURCE"]) }} AS RES_ARCHIVE_SURROGATE_KEY,
        sw1_src.*
    FROM sw1_src
