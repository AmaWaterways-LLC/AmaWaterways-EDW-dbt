...
WITH sw1_src AS (
    ...
    FROM {{ source(var('bronze_source_prefix') ~ '_SW1', 'ACC_BATCH_HEADER') }}
...
sw2_src AS (
    ...
    FROM {{ source(var('bronze_source_prefix') ~ '_SW2', 'ACC_BATCH_HEADER') }}
    -- Incremental load: include only rows whose watermark is greater than the last recorded watermark value
    {% if is_incremental() and not is_full %}
...
QUALIFY
    ROW_NUMBER() OVER (
        PARTITION BY BATCH_HDR_ID, DATA_SOURCE
