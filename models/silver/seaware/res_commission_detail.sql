...
WITH sw1_src AS (
    ...
    FROM {{ source(var('bronze_source_prefix') ~ '_SW1', 'RES_COMMISSION_DETAIL') }}
...
sw2_src AS (
    SELECT
            ...
            _FIVETRAN_DELETED AS SOURCE_DELETED,
            NULL AS COMMENTS
    FROM {{ source(var('bronze_source_prefix') ~ '_SW2', 'RES_COMMISSION_DETAIL') }}
    -- Incremental load: include only rows whose watermark is greater than the last recorded watermark value
    {% if is_incremental() and not is_full %}
...
QUALIFY
    ROW_NUMBER() OVER (
        PARTITION BY RES_COMMISS_DETAIL_ID, DATA_SOURCE
